...
function ConstructTransaction(ApiConnect, Obj) {
    //Trans = {
    //amount: "10.1",
    //fee: "1.1",
  ...
  /*  var TransId = ApiConnect.WalletTransactionsCountGet(Obj.source);
    ...
    var InnerId = NumbToByte1(Trans.id, 6);
    ...
    if (Obj.smart !== undefined && Obj.smart.code !== undefined) {
        var SmartInnerId = NumbToByte1(Trans.id, 6);
    ...
    Trans.amount = new Amount({
        ...
        fraction: Math.trunc(Obj.amount) === Number(Obj.amount)? 0 : Number(String(Obj.amount).split(".")[1]) * Math.pow(10, 17)
    });
...
function Fee(v) {
    ...
    return { exp: exp + 18, man: Number((v * 1024).toFixed(0)) };
}
...
function NumbToByte1(numb, CountByte) {
  return Conversion.Uint32_To_Byte_Array(numb)
}
